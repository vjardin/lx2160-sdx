...
- `lx2160-sdx` — the generic SerDes walker: PLL / lane / CDR state
  checks
- `lx2160-pcie` — PCIe controller/slot state: which PEX1..6 the
  latched RCW enables.
...
A single c file, dependency-free tool that walks the LX2160A's three SerDes
blocks (SD1/SD2/SD3) over `/dev/mem`, it decodes:
...
`[!!]` flags every lane whose CDR loop hasn't latched onto an incoming signal
...
Per-lane state:
  Lane 6 (LNG)  [OK]  --  USXGMII13 (LAN8023)
    ...
    TX equ  = TECR0=0x...  TECR1=0x...  (FIR taps; refer to lx2160 RM 26.4.1.15)
